...
`src/App.tsx` → renders `Header`/`Footer` layout with `react-router` `BrowserRouter`. Routes defined in `src/router/index.ts` map paths to page components, lazy-loaded with `React.lazy()` + `<Suspense>`. Imports come from `react-router` directly — `react-router-dom` was a mirror package that upstream froze at 7.18.2 and **removed** in v8; importing from `react-router` is the sanctioned path.
...
- **ci.yml** job DAG — **ordinary push/PR:** `changes` → `static-check` → (`test`, `integration-test`, `build` parallel) → `ci-pass` (aggregator). **Tag push additionally:** `security` (off `changes`), `e2e`, `dast`, `docker-scan`, then **`docker-publish`** (`needs: [changes, docker-scan, e2e, dast]`) — five release-only jobs. **Build/scan and publish are SEPARATE jobs (split 2026-07-29):** previously one `docker` job did both and did *not* `need` `e2e`/`dast`, so on a tag the image, `:latest`, the SBOM and a permanent Rekor signature went public in parallel with those gates — a red `ci-pass` afterwards cannot unpublish. All release jobs now gate on explicit **`needs.<job>.result == 'success'`**, not `!failure() && !cancelled()`: the latter deliberately permits a **skipped** prerequisite, and also a **`cancelled`** one — which is how a job that hits its own `timeout-minutes` is reported. `security` has `timeout-minutes: 15` and depends on external feeds (Trivy DB, npm audit), so a slow feed could time it out and still publish. `ci-pass` additionally asserts, **on tags only**, that all five release jobs are `success` — because `contains(needs.*.result,'failure')` scores a **skipped** job as PASS, making the aggregator structurally blind to a silently-skipped release job. A lightweight `mermaid-lint` job also runs off `changes` for **docs-only** changes — README mermaid blocks are docs-excluded from `code`, so `static-check`'s mermaid-lint never sees them; on code changes `static-check` covers mermaid instead, so the job is gated `docs == 'true' && code != 'true'`.
...
- **Dockerfile.prod**: Three-stage build — (1) `node:24-alpine` builder produces the Vite bundle; (2) `caddy:2.11.4-builder-alpine` runs `xcaddy build v2.11.4` with `GOTOOLCHAIN=go1.26.5` to rebuild Caddy's stdlib past three Go CVEs (the vanilla `caddy:2.11.4-alpine` binary ships Go 1.26.3 and is flagged for CVE-2026-42504 MIME DoS + CVE-2026-27145 crypto/x509 DoS, both fixed in 1.26.4, **and** CVE-2026-39822 `os.Root` symlink traversal, which needs **1.26.5** — verified via `trivy image` on 2026-07-29: all three are absent from the built image). Caddy 2.11.4 already pins go-jose/v3 v3.0.5, so the earlier `--replace` workaround for CVE-2026-34986 is retired — but two **new** `--replace` flags patch transitive CVEs in modules caddy 2.11.4 declares as *indirect* deps and that no caddy release yet fixes: `golang.org/x/text`→**v0.40.0** (CVE-2026-56852) and `google.golang.org/grpc`→**v1.82.1** (GHSA-hrxh-6v49-42gf). Both floors are Renovate-tracked via a `--replace` customManager; the rebuilt image scans **0 CVEs** on both the alpine layer and the gobinary; (3) `caddy:2.11.4-alpine` runtime, with the rebuilt binary copied in over the bundled one, `gettext` added for `start-caddy.sh`'s envsubst pass, `cap_net_bind_service` stripped from `/usr/bin/caddy` (so the binary execs cleanly under K8s `capabilities.drop:[ALL]`), non-root `USER ${APP_UID}:${APP_GID}` (default `1000:1000`, build-arg overridable). The listen port is single-sourced through `ARG APP_INTERNAL_PORT=8080` → `ENV PORT` → Caddyfile `:{$PORT:8080}` → `EXPOSE`, so the K8s ConfigMap `PORT` actually drives the listen port. A `HEALTHCHECK` probes `/internal/isalive` via busybox `wget` (literal flag timings, `${HEALTHCHECK_HOST}:${PORT}` CMD body). All three stages pin base images by SHA256 digest. OCI labels (artifacthub, vendor, license) on the runtime stage. See `Dockerfile.prod` for the pinned tags.
...
- [x] **`.trivyignore` waivers for 3 base-image OS CVEs** — ~~`CVE-2026-45447` (openssl), `CVE-2026-45186` (libexpat), `CVE-2026-6732` (libxml2)~~ **RESOLVED 2026-07-29: removed as dead, not re-dated.** Measured with the waivers explicitly disabled (`trivy image --ignorefile /dev/null`) against both the base and the built image: alpine 3.23.5 now ships patched openssl (libcrypto3/libssl3 3.5.7-r0, nothing newer offered by `apk upgrade --simulate`), and libexpat/libxml2 are **not installed in the image at all**. All three IDs return zero hits. The built prod image scans **0 on the alpine layer** — the five CVEs the base still carries (c-ares `CVE-2026-33630`, curl/libcurl `CVE-2026-5773` + `CVE-2026-6276`) are all cleared by `Dockerfile.prod`'s `apk --no-cache upgrade`.
- [x] **Two caddy-dependency CVEs would have failed the next release tag's Trivy image gate** — **RESOLVED 2026-07-29.** `usr/bin/caddy` carried `golang.org/x/text` v0.37.0 (CVE-2026-56852, fixed 0.39.0) and `google.golang.org/grpc` v1.81.0 (GHSA-hrxh-6v49-42gf, fixed 1.82.1), both **indirect** deps in caddy 2.11.4's own `go.mod`. 2.11.4 is the latest caddy release, so no bump was available; fixed with two `xcaddy --replace` flags (x/text → v0.40.0, grpc → v1.82.1) — the same mechanism the retired go-jose workaround used. Verified: `trivy image --ignore-unfixed --ignorefile /dev/null` on the rebuilt image reports **0 vulnerabilities on BOTH the alpine layer and the caddy gobinary**; `make docker-smoke-test`, `make e2e` (44/44) and `make e2e-browser` (4/4) all pass, so the rebuilt binary genuinely serves. Both pinned floors are Renovate-tracked via a new `--replace` customManager so they cannot silently rot when a later advisory raises the floor. **Retire the flags** when a caddy release ships the patched versions itself — check `gh api 'repos/caddyserver/caddy/contents/go.mod?ref=<tag>'` and drop them if both are already at/above the floors.
