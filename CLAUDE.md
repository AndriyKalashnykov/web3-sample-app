...
- [ ] **Two caddy-dependency CVEs will fail the next release tag's Trivy image gate** — the built image is clean on the alpine layer and on the Go stdlib (the `GOTOOLCHAIN=go1.26.5` xcaddy rebuild clears all three stdlib CVEs), but `usr/bin/caddy` still carries **`golang.org/x/text` v0.37.0 → CVE-2026-56852** (fixed 0.39.0) and **`google.golang.org/grpc` v1.81.0 → GHSA-hrxh-6v49-42gf** (fixed 1.82.1). These come from caddy 2.11.4's own `go.mod`, so the rebuild inherits them. Measured 2026-07-29 via `trivy image --severity CRITICAL,HIGH --ignore-unfixed` on the locally-built image: **2 HIGH**. The `docker` job's scan is `--exit-code 1`, so the next `make release` **will go red**. Not urgent (the job is tag-gated, so ordinary commits are unaffected) and **not yet fixed** — the likely fix is `xcaddy build ... --replace` for both modules, the same pattern the retired go-jose workaround used, or waiting for a caddy release that bumps them. Re-measure before acting; a caddy patch release may clear it for free.
